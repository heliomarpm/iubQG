...
export abstract class BaseService<T> {
	...
	getById(id: number, withAuth: boolean = false): Observable<T> {
		return this.http
			.get<T>(`${this.baseUrl}/${id}`, { headers: this.getHeaders(withAuth) })
			.pipe(catchError(this.errorHandler.handleError));
	}

	create(data: T, withAuth: boolean = false): Observable<T> {
		return this.http
			.post<T>(`${this.baseUrl}`, data, { headers: this.getHeaders(withAuth) })
			.pipe(catchError(this.errorHandler.handleError));
	}

	update(id: number, data: T, withAuth: boolean = false): Observable<T> {
		return this.http
			.put<T>(`${this.baseUrl}/${id}`, data, { headers: this.getHeaders(withAuth) })
			.pipe(catchError(this.errorHandler.handleError));
	}

	delete(id: number, withAuth: boolean = false): Observable<void> {
		return this.http
			.delete<void>(`${this.baseUrl}/${id}`, { headers: this.getHeaders(withAuth) })
			.pipe(catchError(this.errorHandler.handleError));
	}
