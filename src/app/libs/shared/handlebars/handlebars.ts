...
export function normalizeTemplate(template: string, reservedWords: Set<string>): string {
	...
	// Normaliza nomes de helpers/blocos e subexpressões (ex: (Eq ...))
	return normalize.replace(/([({{#\/])(\w+)/g, (match, prefix, name) => {
		const found = Array.from(reservedWords).find((word) => word.toLowerCase() === name.toLowerCase());
...
export function validateTemplateBlocks(template: string): void {
	...
	while ((match = regex.exec(template))) {
		...
		// Verifica se abertura e fechamento de chaves batem
